...
export function DiscoverFilters({ initialQuery = "" }: { initialQuery?: string }) {
  ...
  const [draftString, setDraftString] = useState(currentString);
  const [query, setQuery] = useState(initialQuery);
  const [isPending, startTransition] = useTransition();
  const draft = useMemo(() => new URLSearchParams(draftString), [draftString]);
  const include = new Set(draft.getAll("include"));
  const exclude = new Set(draft.getAll("exclude"));

  function updateDraft(change: (next: URLSearchParams) => void) {
    const next = new URLSearchParams(draftString);
    change(next);
    next.delete("page");
    setDraftString(next.toString());
  }
  ...
  const hasDraftFilters = Boolean(query.trim() || draftString);
  ...
  return (
    <aside className={`filter-panel${isPending ? " is-pending" : ""}`} aria-label="Bộ lọc truyện" aria-busy={isPending}>
      <div className="filter-panel__title">
        ...
        <strong>Lọc sâu</strong>
        <span>{isPending ? "đang tìm…" : "chọn xong rồi bấm tìm"}</span>
      </div>
      ...
      <div className="filter-group">
        ...
        <div className="filter-chips filter-chips--single">
          {moods.map(([slug, label]) => (
            <button key={slug} type="button" data-state={draft.get("mood") === slug ? "include" : "idle"} onClick={() => setValue("mood", draft.get("mood") === slug ? "" : slug)} aria-pressed={draft.get("mood") === slug}>
              {draft.get("mood") === slug ? <Check aria-hidden="true" /> : null}{label}
            </button>
          ))}
        </div>
      </div>
      <div className="filter-selects">
        <label><span>Trạng thái</span><select data-filter="status" value={draft.get("status") ?? ""} onChange={(event) => setValue("status", event.target.value)}><option value="">Tất cả</option><option value="ongoing">Đang ra</option><option value="completed">Hoàn thành</option></select></label>
        <label><span>Định dạng</span><select data-filter="format" value={draft.get("format") ?? ""} onChange={(event) => setValue("format", event.target.value)}><option value="">Tất cả</option><option value="format-webtoon">Webtoon / Manhwa</option><option value="format-manga">Manga</option><option value="format-manhua">Manhua</option></select></label>
        <label><span>Nhịp truyện</span><select data-filter="pace" value={draft.get("pace") ?? ""} onChange={(event) => setValue("pace", event.target.value)}><option value="">Mọi nhịp</option><option value="pace-fast">Nhanh, vào việc sớm</option></select></label>
        <label><span>Độ dài hiện có</span><select data-filter="max-chapters" value={draft.get("maxChapters") ?? ""} onChange={(event) => setValue("maxChapters", event.target.value)}><option value="">Không giới hạn</option><option value="50">Dưới 50 chương</option><option value="100">Dưới 100 chương</option><option value="300">Dưới 300 chương</option></select></label>
        <label><span>Điểm tối thiểu</span><select data-filter="min-score" value={draft.get("minScore") ?? ""} onChange={(event) => setValue("minScore", event.target.value)}><option value="">Không giới hạn</option><option value="3.5">3.5 sao</option><option value="4">4 sao</option><option value="4.25">4.25 sao</option></select></label>
        <label><span>Sắp xếp</span><select data-filter="sort" value={draft.get("sort") ?? "latest"} onChange={(event) => setValue("sort", event.target.value)}><option value="latest">Mới cập nhật</option><option value="rating">Đánh giá cao nhất</option><option value="relevance">Tên phù hợp nhất</option><option value="shortest">Ít chương trước</option></select></label>
      </div>
      <div className="filter-legend"><span><Check aria-hidden="true" /> phải có</span><span><Minus aria-hidden="true" /> loại trừ</span></div>
      <div className="filter-actions">
        <button className="button button--ink" type="button" onClick={applyFilters} disabled={isPending}>
          <Search aria-hidden="true" /> {isPending ? "Đang lọc toàn bộ chỉ mục…" : "Tìm truyện theo bộ lọc"}
        </button>
        {hasDraftFilters ? <button type="button" onClick={resetFilters}>Xóa bộ lọc</button> : null}
      </div>
    </aside>
  );
}
