...
export async function RatingPanel({
  ...
  const isProvisional = !rating.score5 && Boolean(fallbackScore && fallbackSource?.startsWith("Điểm Mực"));
  ...
  return (
    <aside className="score-panel" aria-label="Đánh giá và nguồn">
      <p className="section-kicker">{rating.isAggregate ? "Điểm tổng hợp" : isProvisional ? "Điểm Mực tạm tính" : "Điểm có nguồn"}</p>
      {score ? (
        <><div className={`score-panel__number${isProvisional ? " score-panel__number--provisional" : ""}`}><Star aria-hidden="true" />{isProvisional ? "~" : ""}{score.toFixed(1)}<small>/5</small></div><p>{scoreSource}</p></>
      ) : (
        <><div className="score-panel__number score-panel__number--empty">—</div><p>Chưa đủ nguồn đánh giá đáng tin để tính điểm.</p></>
      )}
      <div className="confidence-row"><ShieldCheck aria-hidden="true" /><span>{rating.score5 ? `${ratingConfidenceLabel(rating.confidence)} · ${rating.voteCount.toLocaleString("vi-VN")} lượt chấm` : isProvisional ? "Chưa khớp điểm công khai; ký hiệu ~ giúp phân biệt điểm tạm tính" : "Không biến thiếu dữ liệu thành 0 sao"}</span></div>
...
export function RatingPanelFallback() {
  return (
    <aside className="score-panel score-panel--loading" aria-label="Đang tổng hợp đánh giá">
      ...
      <p>AniList, Kitsu và MyAnimeList đang được tải riêng để phần nội dung mở trước.</p>
    </aside>
  );
}
